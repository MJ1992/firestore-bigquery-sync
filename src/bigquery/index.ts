...
export interface FirestoreBigQueryEventHistoryTrackerConfig {
  ...
  transformFunction?: (row : any[]) => [] | undefined;
 ...
 * - Table: Raw Data  table {@link FirestoreBigQueryEventHistoryTracker#rawDataTableName}.
//  * - View: Latest view {@link FirestoreBigQueryEventHistoryTracker#}.
  ...
  implements FirestoreEventHistoryTracker
{
  ...
  private async transformRows(rows: any[]) {
    if (this.config.transformFunction) {
      return this.config.transformFunction(rows);
    }
    return rows;
  }
    ...
    return new Promise((resolve) => {
      let handle = setInterval(async () => {
        try {
          ...
          const rawDataTableName = this.rawDataTableName();
          const table = dataset.table(rawDataTableName);
  ...
  private async insertData(
    ...
    try {
      const dataset = this.bigqueryDataset();
      const table = dataset.table(this.rawDataTableName());
  ...
  /**
   * Creates the raw data table if it doesn't already exist.
   */

  private async initializeDataTable() {
    const dataTableName = this.rawDataTableName();
    const dataset = this.bigqueryDataset();
    const table = dataset.table(dataTableName);
    const [tableExists] = await table.exists();
    ...
    if (tableExists) {
      ...
      if (!documentIdColExists) {
        ...
        logs.addNewColumn(this.rawDataTableName(), documentIdField.name);
      }
      if (!pathParamsColExists && this.config.wildcardIds) {
        fields.push(documentPathParams);
        logs.addNewColumn(
          this.rawDataTableName(),
      ...
      if (shouldUpdate) {
        ...
        logs.updatingMetadata(this.rawDataTableName(), {
          config: this.config,
          documentIdColExists,
          pathParamsColExists
        });
      ...
      logs.bigQueryTableCreating(dataTableName);
      const schema = { fields: [...this.tableSchema.fields] };
      ...
      try {
        ...
        logs.bigQueryTableCreated(dataTableName);
      } catch (ex) {
        logs.tableCreationError(dataTableName, ex.message);
      }
    }
  ...
  private rawDataTableName(): string {
    return `${this.config.tableId}_data`;
  }
